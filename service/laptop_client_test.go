...
func TestClientCreateLaptop(t *testing.T) {
	...
	// To check the laptop is saved to the store
	other, err := laptopStore.Find(res.Id)
	require.NoError(t, err)
	require.NotNil(t, other)

	// check that the saved laptop is the same as the one we sent
	requireSendLaptop(t, laptop, other)
}
...
func TestCientUploadImage(t *testing.T) {
	t.Parallel()
...
func TestCientRateLaptop(t *testing.T) {
	t.Parallel()
...
func startTestLaptopServer(t *testing.T, laptopStore service.LaptopStore, imageStore service.ImageStore, ratingStore service.RatingStore) string {
	...
	go grpcServer.Serve(listener) //block code
...
func requireSendLaptop(t *testing.T, laptop1, laptop2 *pb.Laptop) {
